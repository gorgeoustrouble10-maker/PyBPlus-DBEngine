...
- **Checkpoint**：`DatabaseContext.checkpoint_all()` 对每个表的 WAL 调用 `wal.log_checkpoint()`，再 `truncate_wal_after_checkpoint(path)`，仅保留最后 CHECKPOINT 之后的内容。
- **时机**：可由后台线程或手动调用；当前实现为内存表 + 每表 WAL，无 BufferPool 刷脏。
